
using System.Text;
using Microsoft.JSInterop;
using SpawnDev.BlazorJS;
using SpawnDev.BlazorJS.JSObjects;

namespace SpawnDev.BlazorJS.THREE
{
    public class PointLightHelper : Object3D
    {
        #region Constructors
        /// <inheritdoc/>
        public PointLightHelper(IJSInProcessObjectReference _ref) : base(_ref) { }
        
        /// <summary>
        /// constructor(light: PointLight, sphereSize?: number, color?: ColorRepresentation);
        /// </summary>
        public PointLightHelper(PointLight light, float sphereSize, ColorRepresentation color) : base(JS.New("THREE.PointLightHelper", light, sphereSize, color)) { }
        
        #endregion

        #region Properties
        /// <summary>
        /// color: ColorRepresentation | undefined;
        /// </summary>
        public ColorRepresentation Color { get => JSRef!.Get<ColorRepresentation>("color"); set => JSRef!.Set("color", value); }
        /// <summary>
        /// light: PointLight;
        /// </summary>
        public PointLight Light { get => JSRef!.Get<PointLight>("light"); set => JSRef!.Set("light", value); }
        /// <summary>
        /// matrix: Matrix4;
        /// </summary>
        public Matrix4 Matrix { get => JSRef!.Get<Matrix4>("matrix"); set => JSRef!.Set("matrix", value); }
        /// <summary>
        /// override matrixAutoUpdate: boolean;
        /// </summary>
        public bool MatrixAutoUpdate { get => JSRef!.Get<bool>("matrixAutoUpdate"); set => JSRef!.Set("matrixAutoUpdate", value); }
        /// <summary>
        /// override readonly type: string | "PointLightHelper";
        /// </summary>
        public string Type { get => JSRef!.Get<string>("type"); }
        #endregion

        #region Methods
        /// <summary>
        /// update(): void;
        /// </summary>
        public void Update()
        {
            JSRef!.CallVoid("update");
        }
        /// <summary>
        /// dispose(): void;
        /// </summary>
        public void DisposeJS()
        {
            JSRef!.CallVoid("dispose");
        }
        #endregion
    }
}
